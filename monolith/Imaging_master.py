...
from by_splitmethod import method_1
...
def execute_id_filter():
    ...
    if input_text:
        ...
        print(f"Executing query: {query} with items: {items}")  # Debugging Query
        try:
            cursor.execute(query, *items)
            rows = cursor.fetchall()

            # Debug: check how many rows are returned
            print(f"Rows fetched from database: {len(rows)}")
            ...
            for row in rows:
                ...
                print(f"Processing row: ID={id_value}, Desc={desc_value}, Name={name_value}")
                ...
                if method_result and len(method_result) >= 2:
                    ...
                    # Debug: Check the method result
                    print(f"method_1 returned Folder: {folder_value}, File: {file_value}")
                    ...
                    if id_value not in inserted_ids:
                        ...
                        print(f"Inserted ID {id_value} into Treeview.")
                    else:
                        print(f"ID {id_value} already inserted.")
                else:
                    print(f"method_1 returned invalid result for Desc: {desc_value}")
...
def to_id_folder():
    if inserted_ids:
        ...
        folder_path.set(selected_files)
        if selected_files:
            for row_id in tree.get_children():
                ...
                for image in images_loaded:
                    ...
                    if image_name.startswith(row_data_file):
                        ...
                        print(f"Moved {image}")
                        updated_row_data = ("📦→ID", row_data[1], row_data[2], row_data[3], row_data[4], row_data[5])
            ...
            try:
                ...
                for row in rows:
                    ...
                    if method_result and len(method_result) >= 2:
                        ...
                        updated_row_data = ("📦→UPC", row[0], row[1], row[2], folder_value, file_value)
                        for row_id in tree.get_children():
                            row_data = tree.item(row_id, "values")
                            if row_data[1] == id_value:
                                tree.item(row_id, values=updated_row_data)
                        for current_dir, subdirs, files in os.walk(selected_files, topdown=False):
                            if not files:
                                os.rmdir(current_dir)
                                print('deleted ' + current_dir)
                            else:
                                print(f'no modification {current_dir}')
                    else:
                        print(f"Invalid method_1 result for Desc: {desc_value}")
            except Exception as e:
...
def export_treeview():
    with open('records.txt', 'a', encoding='utf-8') as record_file:
        for row_id in tree.get_children():
            row_data = tree.item(row_id, "values")
            record_file.write(f'{row_data}\n')
